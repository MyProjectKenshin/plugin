...
class RssAntennaPlugin
{
    ...
    public function __construct()
    {
        ...
        $rss_number_link = $options['feed_count_link'] > 0 ? $options['feed_count_link'] : 5;
        add_action('admin_init', array(&$this, 'on_admin_init'));
        add_action('admin_menu', array(&$this, 'on_admin_menu'));
        add_action('wp_enqueue_scripts', array(&$this, 'on_enqueue_scripts'));
        for ($i = 1; $i <= $rss_number_link; $i++) {
            add_shortcode(self::SHORTCODE . $i, array(&$this, 'show_shortcode'));
        }
        $this->count_link = $rss_number_link;
    ...
    function on_admin_init()
    {
        ...
        add_settings_field('rss_number_link', 'Number Rss Link', array(
            &$this,
            'setting_number_link'
        ), __FILE__, 'main_section');
        add_settings_field('id_description', '記事の抜粋を表示する', array(
            &$this,
            'setting_description_chk'
        ), __FILE__, 'main_section');
        add_settings_field('id_image', '　サムネイル画像を表示する', array(
            &$this,
            'setting_image_chk'
        ), __FILE__, 'main_section');
        add_settings_field('id_image_position', '　サムネイル画像表示位置', array(
            &$this,
            'setting_image_position'
        ), __FILE__, 'main_section');
        add_settings_field('id_adblock', '広告を表示しない', array(
            &$this,
            'setting_adblock_chk'
        ), __FILE__, 'main_section');
    ...
    {
        add_options_page("RSS Antenna設定", "RSS Antenna設定", 'administrator', __FILE__, array(
            &$this,
            'show_admin_page'
        ));
    }
    ...
    public function show_admin_page()
    {
        ...
        $shortcode = '';
        for ($i = 1; $i <= $this->count_link; $i++) {
            $shortcode .= "[" . self::SHORTCODE . $i . "]" . ' - ';
        }
        include_once(dirname(__FILE__) . '/admin-view.php');
    }

    function show_rss_antenna($tag)
    {

        $posi = substr($tag, 7);
        $info = new RssInfo(self::OPTION_NAME, $posi);
        include(dirname(__FILE__) . '/rss-antenna-view.php');
    }

    function show_shortcode($atts = [], $content = null, $tag = '')
    {
        ob_start();
        $this->show_rss_antenna($tag);
    ...
    function setting_number_link()
    {
        $options = get_option(self::OPTION_NAME)['feed_count_link'];
        echo "<input value = '$options' id='rss_number_link' name='rss_antenna_options[feed_count_link]'>";
    }
...
class RssInfo
{
    ...
    public function __construct($option_name, $posi = false)
    {
        $this->posi = $posi;
    ...
    private function createItems()
    {
        ...
        $feed_url[] = $feed_urls[$this->posi - 1];
//        var_dump($feed_url); die;

        global $wp_version;
        if (version_compare($wp_version, '3.5', '>=')) {
            $rss = $this->fetch_feed($feed_url);
        } else {
